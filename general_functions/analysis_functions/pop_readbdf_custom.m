...
function [EEG, command] = pop_readbdf_custom(filename, blockrange, eventchans, ref, delchan); 
EEG = [];
command = '';
...
if ~isempty(eventchans)
    ...
    % code adjustment for combination of 2 trigger input: ProPixx trigger + lpt from computer
    % I see a lot of problems:
    % diff doesn't work when at sample point s triger 1 is sent (e.g. value
    % 3) and at point s+1 trigger 2 is sent (e.g. value 8) and
    % simultaneously trigger 1 is set to 0 --> diff then creates trigger
    % value 5!
    % solution: trigger sending time (before pin is set to 0) needs to be
    % smaller than 1/(2*srate) --> should work with lower sampling rates!
    trigger_vect = diff([0 mod(EEG.data(end,:),256*256)]); % difference between values to index offsets
    trigger_vect_abs = trigger_vect;
    trigger_vect_abs(trigger_vect_abs<0) = 0; % set all negative values to 0
    trigger_vect_abs_index = find(trigger_vect_abs); % get indices of events
    for trig_i = 1:numel(trigger_vect_abs_index) % loop across events
        EEG.event(end+1).latency = trigger_vect_abs_index(trig_i);
        EEG.event(end).type = trigger_vect_abs(trigger_vect_abs_index(trig_i));
    end
%         
