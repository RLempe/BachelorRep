...
% MODIFIED VERSION (M. Dotzer 18-07-12):
% automatically seperates simultaneous triggers sent via parallel port OR Datapixx
% using bdf_trigger_sort_mod.m 
...
function [EEG, command] = pop_readbdf_mod(filename, blockrange, eventchans, ref, delchan); 
EEG = [];
command = '';
...
if ~isempty(eventchans)
    ...
    %end;
    thiscode = 0;
    allcodes = zeros(2,size(EEG.data,2)-1);
    for p = 1:size(EEG.data,2)-1
        prevcode = thiscode;
        thiscode = mod(EEG.data(end,p),256*256);   % andrey's code - 16 bits 
        allcodes(1,p) = p; allcodes (2,p) = thiscode;
...
%         end;
    ...
    % use custom trigger sorting function (local function below)
    disp([ 'Custom trigger function... Might take a while...' ]);
    EEG = bdf_trigger_sort_mod(allcodes,EEG);
