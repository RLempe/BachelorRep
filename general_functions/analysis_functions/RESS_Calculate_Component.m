...
%RESS_CALCULATE_COMPONENT Function to calculate RESS component
...
timelims = [EEG_in.xmin EEG_in.xmin]./1000; % default time limits dependent on input data
neighparams = repmat({[-neighfreq neighwidt;neighfreq neighwidt]},numel(freqs),1);
ress_v = 'v.2018-06-05';                    % current version
...
for i_freq = 1:numel(freqs)
    ...
    fdatLo = filterFGx(data,EEG_in.srate,freqs(i_freq)+neighparams{i_freq}(1,1),neighparams{i_freq}(1,2));
    ...
    fdatHi = filterFGx(data,EEG_in.srate,freqs(i_freq)+neighparams{i_freq}(2,1),neighparams{i_freq}(2,2));
    ...
    ress_ts = zeros(EEG_in.pnts,size(data,3));
    ...
    noress_ts = zeros(size(data));
    for ti=1:size(data,3)
        noress_ts(:,:,ti)  = compmaps(:,setdiff(1:size(evecs,2),comp2plot)) * evecs(:,setdiff(1:size(evecs,2),comp2plot))' * data(:,:,ti);
    end
    ...
    snr_fdatLo = filterFGx(snr_data,EEG_in.srate,freqs(i_freq)+neighparams{i_freq}(1,1),neighparams{i_freq}(1,2));
    snr_fdatHi = filterFGx(snr_data,EEG_in.srate,freqs(i_freq)+neighparams{i_freq}(2,1),neighparams{i_freq}(2,2));
    ...
    snr_snr_ind = mean(snr_fdatAt_fft_ind(dsearchn(snr_freqs',freqs(i_freq)-peakwidt/2):dsearchn(snr_freqs',freqs(i_freq)+peakwidt/2)))...
        /mean([...
        mean(snr_fdatLo_fft_ind(dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(1,1)-neighparams{i_freq}(1,2)/2):...
        dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(1,1)+neighparams{i_freq}(1,2)/2))) ...
        mean(snr_fdatHi_fft_ind(dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(2,1)-neighparams{i_freq}(2,2)/2):...
        dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(2,1)+neighparams{i_freq}(2,2)/2)))...
        ]);
    snr_snr_evo = mean(snr_fdatAt_fft_evo(dsearchn(snr_freqs',freqs(i_freq)-peakwidt/2):dsearchn(snr_freqs',freqs(i_freq)+peakwidt/2)))...
        /mean([...
        mean(snr_fdatLo_fft_evo(dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(1,1)-neighparams{i_freq}(1,2)/2):...
        dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(1,1)+neighparams{i_freq}(1,2)/2))) ...
        mean(snr_fdatHi_fft_evo(dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(2,1)-neighparams{i_freq}(2,2)/2):...
        dsearchn(snr_freqs',freqs(i_freq)+neighparams{i_freq}(2,1)+neighparams{i_freq}(2,2)/2)))...
        ]);
