...
% (c) 2018      - C.Gundlach  
function [AdjConditions]=ERP_IsolCol(Colors,TrialsPerCon,MaxStd,Shift,varargin)
sca
...
RestrictKeysForKbCheck([]); % allow input from all buttons
...
while any(LumAdj==0)
    ...
    for n1=1:size(Col2Pre,1)
        Text2Disp = [Text2Disp '\n\n' sprintf('col [%1.0f %1.0f %1.0f]:',Col2Pre(n1,:)),...
            ' ' sprintf(' %5.1f',TmpLumAdj(n1,:)) sprintf(': M = %5.2f STD = %5.2f ',mean(TmpLumAdj(n1,:),2),LumAdjStd(n1)) ...
            sprintf('NewColor = [%1.4f %1.4f %1.4f]',ScaleLuminance(Col2Pre(n1,:),LumAdj(n1)))];
    end
    % display results
    if any(LumAdjStd>MaxStd)
         LumAdj(LumAdjStd>MaxStd)=0; % set adjusted luminance values to 0 for values higher than cutoff
...
AdjConditions=[ScaleLuminance(Col2Pre(1:end,:),LumAdj)]; %dont output background color
ListenChar(0)
sca
end
